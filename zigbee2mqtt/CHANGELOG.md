...
- ⚠️ **Breaking changes**
    - Zigbee2MQTT configuration is now by default stored in the addon specific configuration directory rather than the main `config` directory. This is required for Home Assistant's
      backup functionality to work correctly. New installations will default to the new path, existing setups will continue to work but need to be manually migrated to make use of Home Assistant's backups:
